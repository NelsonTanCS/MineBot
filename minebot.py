...
class Client(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content == "$minesweeper":
            await minesweeper(message, self)
...
async def minesweeper(message, client):
    ...
    if not canceled:
        await minesweep_create(message, values["Size"], values["Difficulty"])
...
async def get_all_fields(message, client, embed, minesweep):
    ...
    for key, value in parameter_dict.items():
        ...
        canceled, mine_error, values = await get_field(message, client,
                                                       for_items, mine_error,
                                                       values)
...
async def get_field(message, client, for_items, mine_error, values):
    ...
    if not canceled:
        ...
        if reply.content:
            ...
            canceled, mine_error, values = await get_field(
                message, client, for_items, mine_error, values)
    return canceled, mine_error, values
...
async def minesweep_create(message, size, diff):
    ...
    mines.create_map()
    mines.set_bombs()
    text_board = mines.convert_to_text(mines.board)
    if len(text_board) >= 2000:
        split_num = (np.ceil(len(text_board) / 2000)) + 1
        splits = np.array_split(mines.board, split_num)
        for split in splits:
            text = mines.convert_to_text(split)
            try:
...
class Map:
    ...
    def set_bombs(self):
        """Place the bombs."""
        bombs = prob_round(self.size * (self.difficulty + 1))
        placed = 0
        while placed < bombs:
...
async def wait_msg_or_react(message, client):
    """
    ...
    try:
            ...
            [
                client.wait_for("message", timeout=60, check=reply_check),
                client.wait_for("reaction_add", check=react_check),
            ],
